...
function rechercherPannes($connexion, $profil2 = null, $search = '') {
    // Initialiser la clause WHERE
    $whereClause = '';
    $params = [];
    $types = '';
    ...
    if (!empty($search)) {
        $searchConditions = " (p.id LIKE ? OR p.type_panne LIKE ? OR p.date_enregistrement LIKE ? OR 
                               ...
                               u.profil2 LIKE ? OR 
                               o.id LIKE ? OR o.evaluation_qualite LIKE ? OR o.commentaire_suggestion LIKE ?)";

        if ($whereClause) {
            // Si une clause WHERE existe déjà, ajoutez la recherche avec AND
            $whereClause .= " AND $searchConditions";
        } else {
            // Sinon, créez une clause WHERE
            $whereClause = " WHERE $searchConditions";
        }
        
        // Créez le tableau des paramètres pour correspondre au nombre de ? dans la requête
        $params = array_merge($params, array_fill(0, 19, "%$search%"));
        $types .= str_repeat('s', 19); // 's' pour string répété pour chaque paramètre de recherche
    }
...
function allPannes($connexion, $page = 1, $limit = 10, $profil2 = null, $search = '') {
    $offset = ($page - 1) * $limit;

    // Appeler la fonction de recherche
    $pannesFiltrees = rechercherPannes($connexion, $profil2, $search);
